...
from PySide6.QtWidgets import QWidget, QHBoxLayout, QLabel, QToolTip, QApplication, QMainWindow, QVBoxLayout
from PySide6.QtCore import Qt, Signal, QEvent, QObject # Added QObject
...
class ImagePreviewWidget(QWidget):
    ...
    悬停时显示较大的预览，并允许单击删除。
    """
    image_deleted = Signal(int)  # Emits the image_id when deleted (发出删除图像的 image_id)
    
    def __init__(self, image_pixmap: QPixmap, image_id: int, parent: Optional[QWidget] = None):
        super().__init__(parent)
        self.image_pixmap = image_pixmap # This is the thumbnail, original_pixmap is full res
        ...
        self.hover_thumbnail = self._create_hover_thumbnail(thumbnail) # Thumbnail for hover state
        
        self.thumbnail_label.setPixmap(self.display_thumbnail)
        layout.addWidget(self.thumbnail_label)
        
        self.setToolTip("悬停查看大图，点击图标删除图片 (Hover to see larger image, click to delete)")
        self.setMouseTracking(True) # Needed for enterEvent/leaveEvent without mouse button press
        self.preview_window: Optional[QMainWindow] = None # To hold the preview pop-up
    ...
    def _create_hover_thumbnail(self, base_thumbnail: QPixmap) -> QPixmap:
        ...
        painter.fillRect(hover_pixmap.rect(), QColor(255, 100, 100, 160)) # Reddish tint
        painter.end()
        return hover_pixmap
    ...
    def leaveEvent(self, event: QEvent):
        self.is_hovering = False
        self.thumbnail_label.setPixmap(self.display_thumbnail) # Revert to default thumbnail
    ...
    def _show_full_image_preview(self):
        ...
        if self.preview_window and self.preview_window.isVisible(): # Close existing first
        ...
        if preview_pixmap.width() > max_preview_width or preview_pixmap.height() > max_preview_height:
        ...
        self.preview_window.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground) # For rounded corners if QSS is used
