...
from .constants import MAX_IMAGE_WIDTH, MAX_IMAGE_HEIGHT, MAX_IMAGE_BYTES, ContentItem

def process_single_image(pixmap_to_save: QPixmap) -> Optional[Dict[str, Any]]:
    """
    ...
    if current_pixmap.width() > MAX_IMAGE_WIDTH or current_pixmap.height() > MAX_IMAGE_HEIGHT:
    ...
    if not saved_successfully or byte_array.isEmpty(): # 检查 byte_array 是否为空 (Check if byte_array is empty)
        QMessageBox.critical(None, "图像处理错误 (Image Processing Error)", "无法将图像保存为 JPEG 格式。(Could not save image as JPEG.)")
        return None
        
    if byte_array.size() > MAX_IMAGE_BYTES: # 所有尝试后的最终检查 (Final check after all attempts)
        QMessageBox.critical(None, "图像过大 (Image Too Large)", 
    ...
    if not image_data_bytes: # 确保字节数据非空 (Ensure byte data is not empty)
         QMessageBox.critical(None, "图像处理错误 (Image Processing Error)", "无法获取图像数据。(Could not get image data.)")
         return None
    ...
    try:
        ...
        QMessageBox.critical(None, "图像处理错误 (Image Processing Error)", f"图像数据编码失败 (Image data encoding failed): {e}")
        return None
...
def get_image_items_from_widgets(image_widgets: Dict[int, Any]) -> List[ContentItem]:
    ...
    for image_id in list(image_widgets.keys()): 
        ...
        if widget and hasattr(widget, 'original_pixmap'): # 确保 widget 是 ImagePreviewWidget 的实例 (Ensure widget is instance of ImagePreviewWidget)
