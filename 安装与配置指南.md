...
- [开发模式环境变量配置已移除，请使用 UI 设置页面管理 API key](#开发模式环境变量配置已移除请使用-ui-设置页面管理-api-key)
  ...
  - [注意事项](#注意事项)
  - [最新功能特性 ✨](#最新功能特性-)
    - [优化和增强功能 ✨ 全新功能](#优化和增强功能--全新功能)
    - [音频提示功能 ✨ 新增功能](#音频提示功能--新增功能)
    - [窗口截图功能 ✨ 新增功能](#窗口截图功能--新增功能)
    - [显示模式配置 ✨ 新增功能](#显示模式配置--新增功能)
    - [界面布局选择功能](#界面布局选择功能)
    - [文件选择功能](#文件选择功能)
    - [终端窗口功能](#终端窗口功能)
    - [文件拖拽功能](#文件拖拽功能)
...
### 优化和增强功能 ✨ 全新功能
- **文本优化**：一键将口语化输入转换为结构化指令，提高AI理解准确性
- **自定义增强**：支持用户自定义增强指令，按需处理文本内容
- **多AI提供商**：支持OpenAI、Gemini、DeepSeek、火山引擎等多个AI提供商
- **API密钥管理**：通过UI设置页面统一管理所有提供商的API密钥
...
### 文件选择功能
- **选择文件按钮**：提供专门的文件选择按钮，支持通过对话框选择多个文件
- **智能处理**：自动识别图片文件和普通文件，分别进行相应处理
- **统一体验**：按钮选择的文件与拖拽文件享有相同的处理逻辑和显示效果

### 终端窗口功能
- **多终端支持**：支持PowerShell、Git Bash、Command Prompt三种终端类型
- **hover预览**：鼠标悬停在终端按钮上可预览可用的终端类型
- **嵌入式终端**：点击终端类型后打开独立的嵌入式终端窗口
...
### 设置页面优化
- **统一UI风格**：所有设置项都使用单选按钮形式，保持界面一致性
- **逻辑排序**：设置项按外观主题、界面布局、展示语言、字体大小的逻辑顺序排列
- **实时预览**：布局和主题设置支持实时预览效果
- **API配置**：新增优化功能的API密钥配置和提示词自定义

### UI体验优化
- **智能提示**：输入框焦点状态动态控制提示文字显示
- **窗口固定修复**：修复了取消固定窗口时关闭按钮失效的问题
- **增强提示内容**：提示文字包含完整的操作说明和快捷键
- **响应式布局**：界面支持不同布局模式，并能根据窗口大小自动调整

### 配置和故障排除增强
- **API密钥配置**：详细的多提供商API密钥配置指导
- **优化功能配置**：文本优化和增强功能的配置说明
- **音频功能配置**：音频提示功能的配置和故障排除
- **截图功能说明**：窗口截图功能的使用指导

这些最新功能让 interactive-feedback-mcp 提供了更加专业、灵活和流畅的用户体验。